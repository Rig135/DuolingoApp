...
export function Sidebar() {
  return (
    <aside className="fixed bottom-0 left-0 right-0 z-40 flex h-20 items-center justify-around border-t-2 border-locked-border bg-white dark:bg-[#131f24] md:top-0 md:bottom-auto md:h-screen md:w-64 md:flex-col md:justify-start md:border-r-2 md:border-t-0 md:p-4">
      ...
      <nav className="flex w-full justify-around md:flex-col md:gap-2">
        ...
        <Link href="/settings" className="flex flex-col items-center gap-2 rounded-xl p-2 text-text-muted hover:bg-bg-muted md:flex-row md:p-3">
          <Shield className="h-7 w-7" />
          <span className="hidden font-bold uppercase tracking-wider md:block text-sm">Settings</span>
        </Link>
      </nav>
    </aside>
  )
